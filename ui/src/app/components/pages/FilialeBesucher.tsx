...
export class FilialeBesucher extends Component<any, any> {
  ...
  async fetchItems() {
      ...
      let visitors:any = await getData(`/visitor/branch/${id}`);
     ...
     await this.setState({
        ...
        visitCheck: (visitors.status == 200) ? visitorCheck : [], 
  ...
  async handleSearchSubmit(e: any) {
    ...
    let visitors:any = await getData(`/visitor/branch/${id}?search=${search}`);
    let visitorCheck:any = this.state.visitCheck;
      console.log(visitors);
      if(visitors.status === 200 && visitors.data.response['data'].length > 0) {
        visitors.data.response['data'].forEach((visitor:any, idx:any) => {
                                ...
                                <div className="searchBase search-display">
                                  ...
                                  <div className="input-group">
                                      ...
                                      <div className="input-group-append">
                                        ...
                                        {/*<button className="input-group-text btn" id="basic-addon2" type="submit">senden</button>*/ }
                                      </div>
                                  </div>
                                </div> 
                                </form>
                              ...
                              { !searchBar &&  
                                <div id="navMenu">
                                  <div id="navMenu-wrapper" className="FilialeFilter">
                                    <ul id="navMenu-items">
                                      <div id="menuSelector" />
                                      <li className="navMenu-item"> <Link className="scrollLink" to="/besucher">Alle</Link></li>
                                      { branches['data'] ?
                                        branches['data'].map((branch:any, idx:any) =>
                                          <Fragment key={`${idx}`}>
                                            <li className="navMenu-item"> <NavLink activeClassName="selected active" to={`/filialebesucher/${branch.branchId}`}>{branch.branchName}</NavLink></li>    
                                          </Fragment>
                                        )
                                        : null
                                      }
...
//export default FilialeBesucher
